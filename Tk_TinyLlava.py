...
#model_path = "/home/sujit/Downloads/text-generation-webui-main/models/bczhou_tiny-llava-v1-hf"
model_path = "/home/sujit/Downloads/text-generation-webui-main/models/YouLiXiya_tinyllava-v1.0-1.1b-hf"
#model_path = "/home/sujit/Downloads/text-generation-webui-main/models/llava-hf_llava-1.5-7b-hf"
...
def on_compare():
    if captured_image1 is not None and captured_image2 is not None:
        ...
        if not model_running: Thread(target=moondream, args=(compare_prompt_text, stitched_image, processor)).start()
        
def moondream(prompt_text, frame, processor):
    global model_running, context; model_running = True
    #processor = AutoProcessor.from_pretrained(model_path)
    img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
    prompt="context+<image>"+prompt_text+"\nAssistant:"
    inputs = processor(text=prompt, images=(img), return_tensors='pt').to(0, torch.float16).to(0)
    generator = model.generate(**inputs, max_new_tokens=500, do_sample=True, do_stream=True, stream=True, temperature=0.01, top_p=0.01, top_k=2)

    output = words = "";last_tokens = []
    for index, x in enumerate(generator):
        tokens = x.cpu().tolist()
        word = processor.decode(tokens, skip_special_tokens=False)
        #print(word,end=" ") #debug tokens
        if "�" not in word: 
            a=processor.decode(last_tokens+tokens, skip_special_tokens=False)
            if " " in a:
                word = " " + word
                last_tokens = []
            if "<" in a:
                if ">" not in a:
                    ...
                    last_tokens=[]
                if "<|end_of_turn|>" in a or "<|im_end|>" in a or "</s>" in a:
                    break 
            else:
                last_tokens = tokens
                output = output+word
                #print(word,end="") #debug output
                update_output(output) 

    context=context+"User: "+prompt_text+"\nAssistant: "+output+"\n"
    print("\n"+context)
    model_running = False
...
def on_submit():
    ...
    if not model_running: Thread(target=moondream, args=(prompt_text,prev_frame, processor)).start()
